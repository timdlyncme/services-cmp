...
// Use a simple JWT secret for browser environments
const JWT_SECRET = 'browser_jwt_secret_for_development_only';
...
export class AuthService {
  ...
  async login(email: string, password: string): Promise<{ user: AuthUser; token: string }> {
    try {
      ...
      // In browser environments, we'll use a simple password check
      ...
      const permissionsResult = await dbMock.query(
  ...
  async getUserTenants(userId: string): Promise<Tenant[]> {
    try {
      ...
      const tenantsResult = await dbMock.query(
