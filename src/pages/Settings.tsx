...
const Settings = () => {
  ...
  const loadAzureOpenAISettings = async () => {
    ...
    // Clear connection status when loading new tenant settings
    ...
    try {
      ...
      setAzureOpenAISettings({
        ...
        apiKey: config.api_key ? "********" : "", // Show obfuscated value if API key exists
  ...
  const handleSaveAISettings = async (showToast: boolean = true) => {
    ...
    // Validate required fields - but allow obfuscated API key
    if (!azureOpenAISettings.endpoint || !azureOpenAISettings.deploymentName) {
      toast.error("Please fill in all required fields");
      return;
    }
    
    // If API key is empty (not obfuscated), require it
    if (!azureOpenAISettings.apiKey) {
      toast.error("Please fill in all required fields");
      return;
    }
    
    setIsSavingAISettings(true);
    addLog("Saving and validating Azure OpenAI settings...");
    
    try {
      // First save the configuration
      const configToSave: AzureOpenAIConfigUpdate = {
        api_key: azureOpenAISettings.apiKey === "********" ? undefined : azureOpenAISettings.apiKey, // Don't overwrite if obfuscated
      ...
      await aiAssistantService.updateConfig(configToSave, currentTenant?.tenant_id);
      
      // Then test the connection to validate the saved settings
      addLog("Testing connection with saved settings...");
      const status = await aiAssistantService.checkStatus(currentTenant?.tenant_id);
      ...
      if (status.status === 'connected') {
        ...
        addLog("Azure OpenAI settings saved and validated successfully", "success");
        if (showToast) {
          toast.success("Azure OpenAI settings saved and connection verified");
        }
      } else {
        // Connection failed - show error but don't prevent saving
        setAIConnectionStatus({
          status: status.status,
          message: status.message,
          lastChecked: new Date().toISOString()
        ...
        addLog(`Azure OpenAI settings saved but connection failed: ${status.message}`, "warning");
        if (showToast) {
          toast.warning("Settings saved but connection test failed. Please verify your credentials.");
        }
      }
    } catch (error) {
