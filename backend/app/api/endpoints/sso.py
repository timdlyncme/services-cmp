...
async def handle_sso_callback(
    ...
    try:
        ...
        if provider.provider_type == "azure_ad":
            azure_service = AzureADService(db)
            try:
                logger.info(f"Starting Azure AD callback processing for code: {callback_request.code[:10]}...")
                ...
                # Create SSO session
                logger.info("Creating SSO session...")
                ...
                logger.error(f"Error in Azure AD callback processing: {str(e)}", exc_info=True)
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail=f"Error processing Azure AD callback: {str(e)}"
                )
            finally:
                await azure_service.close()
